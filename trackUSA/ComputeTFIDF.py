...
print("   * * *   Lancement du ComputeTFIDF.py   * * *   ")
print("")
...
print("Instanciation d'un objet corpus")
...
print("Chargement des données corpus")
corpus.LoadData()
...
firstlastdate = corpus.LastDate


# In[20]:


print("Processing des nouvelles données")
...
print("Lancement de la mise à jour TFIDF")
...
docrep = corpus.DocsRepresentation

log = {
"Date" : GetCurrentTime(),
"Run effectué" : corpus.DoesAnalysis,
"Nombre de lignes dataframe de tweets" : len(TweetsToAnalyse),
"Nombre de tweets uniques":corpus.NTweetsUnique,
"Date du dernier retweet enregistré avant analyse" : str(pd.to_datetime(firstlastdate,unit="s")),
"Date du dernier retweet enregistré après analyse" : str(pd.to_datetime(corpus.LastDate,unit="s")),
"TFIDF, Taille du vocabulaire" : len(corpus.COMPTEUR),
"TFIDF, Nombre de tweets scannés" : len(docrep.tweetid.unique())
}

filename = os.path.join(Root,FolderProject,"TFIDF.log")
